			...
			this.items.each( function ( i ) {
				...
				if( self.transformItems ) {
					...
					if( isOldie && ! $.support.transform ) {
						...
						if(i<activeIndex){
                var yScale = 1.2 -((activeIndex - i)*0.1);
                if(isOldie){
                  $(this).css('transform', 'skew(-1deg, -5.4deg) scale(' + yScale + ',' + yScale +') translateX('+(38+((activeIndex-1-i)*15))+'%)');
                }
                else{
                   $(this).css('transform', 'rotateY(10.6deg) scale('+ (0.8 - ((activeIndex-1-i)*0.05)) + ',' + yScale +') translateX('+(38+((activeIndex-1-i)*17))+'%)');
                }
                
               
              }
              else if(i>activeIndex){
                 var yScale =  1.2 -((i - activeIndex)*0.1);
                  if(isOldie){
                    $(this).css('transform', 'skew(1deg,5.4deg) scale(' + yScale + ',' + yScale +') translateX('+(38+((i - activeIndex-1)*15))*-1+'%)');
                  }
                  else{
                    $(this).css('transform', 'rotateY(-10.6deg) scale(' + yScale + ',' + yScale +') translateX('+(38+((i - activeIndex-1)*20))*-1+'%)');
                  }
              }
              else{
